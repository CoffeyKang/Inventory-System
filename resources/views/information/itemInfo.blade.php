...
@elseif(isset($from)&&$from=='receive')
...
@endif
	<fieldset style='text-align:left; color:black; font-weight:900'>
  	...
  	<div class="col-xs-12">
		...
		{{-- description item number and price L --}}
		<div class="col-xs-12">
	...
	{{-- margins --}}
		<div class="col-xs-12">
			...
			<div class="col-xs-4 ">
				...
				<div class="col-xs-6"><span @if($item->price - $item->cost<0)  class='margin_red' @endif>{{number_format($item->price - $item->cost,2)}}</span></div>
			</div>
			<div class="col-xs-4 ">
				<div class="col-xs-6">2nd Margin</div>
				<div class="col-xs-6"><span @if($item->price2 - $item->CADcost<0)  class='margin_red' @endif>{{number_format($item->price2 - $item->CADcost,2)}}</span></div>
			</div>
			<div class="col-xs-4 ">
				<div class="col-xs-6">3rd Margin</div>
				<div class="col-xs-6" ><span @if($item->price3 - $item->CADcost<0)  class='margin_red' @endif>{{number_format($item->price3 - $item->CADcost,2)}}</span></div>
			</div>
		...
		<div class="col-xs-12">
			...
			<div class="col-xs-4 ">
				...
				<div class="col-xs-6"><span @if($item->price4 - $item->CADcost<0)  class='margin_red' @endif>{{number_format($item->price4 - $item->CADcost,2)}}</span></div>
			</div>
			
			<div class="col-xs-4 ">
				<div class="col-xs-6">L Margin</div>
				<div class="col-xs-6" ><span @if($item->pricel - $item->CADcost<0)  class='margin_red' @endif>{{number_format($item->pricel - $item->CADcost,2)}}</span></div>
			</div>
...
@if(isset($_GET['from'])&&$_GET['from']=='receive')
	<div class="col-xs-12" style='font-size:16px;text-align:center'><hr>
			<div class="col-xs-3"><a href="/admin/updateModel?item={{$item->item}}&from=receive" ><b>Edit Model</b></a></div>
			<div class="col-xs-2"><a href="/itemNote?item={{$item->item}}&from=receive">Note</a></div>
			<div class="col-xs-2"><a href="/itemEdit?item={{$item->item}}&from=receive"><b>Edit</b></a></div>
			<div class="col-xs-2"><a href="/createItem1?from=receive"><b>Create</b></a></div>
...
@else
		<div class="col-xs-12" style='font-size:16px;text-align:center'><hr>
			<div class="col-xs-3"><a href="/admin/updateModel?item={{$item->item}}&from=so"><b>Edit Model</b></a></div>
			<div class="col-xs-2"><a href="/itemNote?item={{$item->item}}&from=so">Note</a></div>
			<div class="col-xs-2"><a href="/itemEdit?item={{$item->item}}&from=so"><b>Edit</b></a></div>
			<div class="col-xs-2"><a href="{{url('/createItem1')}}"><b>Create</b></a></div>
			<div class='col-xs-3'><a href="/history/itemHistory?item={{$item->item}}"><b>History</b></a></div>
		</div>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content ">
        ...
        <div class="modal-footer">
          ...
          <a href='{{url("/admin/deleteItem?itemNo=$item->item&from=so")}}' class="btn btn-success" id='doubleCheck'>Delete Item</a>
        </div>
      </div>
    </div>
  </div>
  ...
  {{-- model so --}}
<div class="modal fade" id="myModal_receive" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content ">
        <div class="modal-header   ">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
          {{-- <a href='{{url("/admin/deleteItem?itemNo=$item->item&from=receive")}}' class="btn btn-success" id='doubleCheck'>Delete Item</a> --}}
          <a href='#' class="btn btn-success" id='doubleCheck'>Delete Item</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
	...
	// };
	// for (var i = 0; i >= 3; i++) {
	// 	if ($('.margin')[0].innerText<0) {
	// 		$(this).css('color','red');
	// 		console.log(1);
	// 	}else{
	// 		console.log($('.margin')[i].innerText);
	// 	};
	// };
